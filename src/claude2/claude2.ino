...
const int ZERO_DELAY = 1500;         // Minimal delay for full brightness (microseconds)
...
void runCalibration() {
  ...
  Serial.println("====================================");
  Serial.println("Bulb turned OFF. You can now use the");
  Serial.println("calibration constants in your main code.");
  Serial.println();
...
void calibrateCurrent() {
  ...
  for (int cycle = 0; cycle < CALIBRATION_CYCLES; cycle++) {
    ...
    // Sample current (differential reading)
    for (int i = 0; i < SAMPLE_COUNT; i++) {
